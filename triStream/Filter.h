...
namespace triStream {
  ...
  struct BoxFilter : public Sink {
    typedef std::shared_ptr<BoxFilter> SP;

    static SP create(const Sink::SP &sink, const box3f &domain)
    { return std::make_shared<BoxFilter>(sink,domain); }
      
    BoxFilter(Sink *sink, const box3f &stencil);
    BoxFilter(const Sink::SP &sink, const box3f &stencil);
    BoxFilter(const box3f &stencil, Sink *sink);
    BoxFilter(Sink *sink, const std::vector<box3f> &stencils);
    BoxFilter(const Sink::SP &sink, const std::vector<box3f> &stencils);
    
    void push(const std::vector<Triangle> &triangles) override;

    inline box3f getBox(int which=0) const { return stencils[which]; }
  private:
